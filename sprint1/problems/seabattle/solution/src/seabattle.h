...
public:
    // Перечисление State определяет возможные состояния клетки на игровом поле: 
    // UNKNOWN (неизвестно), EMPTY (пусто), SHIP (корабль) и KILLED (уничтожено). 
    enum class State {
        UNKNOWN,
        EMPTY,
        KILLED,
    ...
    static const size_t field_size = 8; // Размер игрового поля (8x8)

    // Конструктор класса SeabattleField инициализирует игровое поле, заполняя его заданным состоянием по умолчанию (по умолчанию UNKNOWN).
    SeabattleField(State default_elem = State::UNKNOWN) {
        field_.fill(default_elem);
    }

    // Метод GetRandomField генерирует случайное игровое поле с размещёнными кораблями, используя переданный генератор случайных чисел.
    ...
    // Метод обьявлен static потому что он не зависит от конкретного экземпляра класса SeabattleField и может быть вызван без создания объекта этого класса.
    template <class T>
...
public:    
    ...
    ShotResult Shoot(size_t x, size_t y) {
        // Если в клетке (x, y) нет корабля, возвращаем результат промаха (MISS).
        if (Get(x, y) != State::SHIP) return ShotResult::MISS;

        // Если в клетке (x, y) есть корабль, помечаем её как поражённую (KILLED) и уменьшаем вес кораблей на единицу.
        Get(x, y) = State::KILLED;
        --weight_;

        // Если после поражения корабля он полностью уничтожен (все его клетки поражены), возвращаем результат уничтожения (KILL).
        // В противном случае возвращаем результат попадания (HIT).
    ...
    // Метод IsLoser возвращает true, если все корабли на поле уничтожены, то есть вес кораблей равен нулю.
    bool IsLoser() const {
        return weight_ == 0;
    }
